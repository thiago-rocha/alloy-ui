...
A.LayoutRow = A.Base.create('layout-row', A.Base, [], {
    ...
    _resizeColsAfterRemoving: function(cols, index) {
        ...
        this._resizeNeighborColsWidth(neighbor, removedColSize, +1);
    ...
    _resizeColsAfterAdding: function(cols, index) {
        ...
        this._resizeNeighborColsWidth(neighbor, colSize, -1);
     ...
     *
     * @method _resizeNeighborColsWidth
     * @param {Array} cols Columns to be resized
     * @param {Number} size Size to shared between the columns
     * @param {Number} operator Flag indicating if the neighbor
     * will be increase or decrease
     * @protected
     */
    _resizeNeighborColsWidth: function(cols, size, operator) {
        var col1 = cols[0],
            col2 = cols[1];

        while (size) {
            if (col1 && col1.get('size') > 1) {
                col1.set('size', col1.get('size') + operator);
                size --;
            }

            if (col2 && col2.get('size') > 1 && size) {
                col2.set('size', col2.get('size') + operator);
                size --;
            }
        }
    },
